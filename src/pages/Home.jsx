...
function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-blue-100 flex flex-col">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-5'} `}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className={`text-xl font-extrabold tracking-tight block leading-none ${isScrolled ? 'text-slate-900' : 'text-white'} `}>Sipentar</span>
              <span className={`text-[10px] uppercase tracking-widest font-bold ${isScrolled ? 'text-blue-600' : 'text-blue-300'} `}>Desa Lamaran Tarung</span>
            </div>
          </div>

          <div className="hidden md:flex gap-8 items-center">
            <a href="#beranda" className={`text-sm font-semibold transition ${isScrolled ? 'text-slate-600 hover:text-blue-600' : 'text-slate-200 hover:text-white'}`}>Beranda</a>
            <a href="#tentang" className={`text-sm font-semibold transition ${isScrolled ? 'text-slate-600 hover:text-blue-600' : 'text-slate-200 hover:text-white'}`}>Tentang Sipentar</a>
            <a href="#alur" className={`text-sm font-semibold transition ${isScrolled ? 'text-slate-600 hover:text-blue-600' : 'text-slate-200 hover:text-white'}`}>Alur Pelaporan</a>
          </div>

          <div className="flex gap-3">
            <Link to="/login">
              <button className={`hidden sm:block px-5 py-2.5 text-sm font-bold rounded-lg transition shadow-sm border ${isScrolled ? 'text-slate-700 bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50' : 'text-white bg-white/10 border-white/20 hover:bg-white/20 backdrop-blur-md'}`}>
                Masuk
              </button>
            </Link>
            <Link to="/register">
      ...
      <section id="tentang" className="py-24 bg-slate-50 border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            {/* Image / Stats */}
            <div className="order-2 lg:order-1 relative">
              <div className="absolute inset-0 bg-blue-100 rounded-[2rem] transform -rotate-3 transition duration-500"></div>
              <div className="relative bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 text-center">
                    ...
                    <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Akses Terbuka</p>
                  </div>
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 text-center">
                    <p className="text-4xl font-extrabold text-indigo-600 mb-2">100%</p>
                    <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Transparan</p>
                  </div>
            ...
            <div className="order-1 lg:order-2">
              ...
              <div className="space-y-6 text-lg text-slate-600 leading-relaxed font-medium">
                ...
                <div className="flex items-start gap-4 p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-slate-900">Akurasi & Transparansi</h4>
                    <p className="text-sm text-slate-500 mt-1">Setiap laporan diverifikasi melalui NIK dan warga dapat memonitor progres tindak-lanjut secara *real-time*.</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section id="alur" className="py-24 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl font-extrabold text-slate-900 mb-4 tracking-tight">Alur Pelaporan yang Transparan</h2>
            <p className="text-slate-600 text-lg">Sistem ini dirancang untuk memudahkan Anda melapor dalam tiga langkah sederhana tanpa birokrasi yang rumit.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {/* Step 1 */}
            <div className="p-8 rounded-2xl bg-slate-50 border border-slate-100 hover:shadow-lg transition duration-300">
              <div className="w-14 h-14 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center text-xl font-black mb-6">1</div>
              <h3 className="text-xl font-bold text-slate-900 mb-3">Tulis Laporan</h3>
              <p className="text-slate-600 leading-relaxed">Daftarkan identitas NIK Anda, lalu tuliskan rincian kejadian, keluhan, atau aspirasi dengan jelas di form yang tersedia.</p>
            </div>

            {/* Step 2 */}
            <div className="p-8 rounded-2xl bg-slate-50 border border-slate-100 hover:shadow-lg transition duration-300">
              <div className="w-14 h-14 bg-amber-100 text-amber-600 rounded-xl flex items-center justify-center text-xl font-black mb-6">2</div>
              <h3 className="text-xl font-bold text-slate-900 mb-3">Proses Verifikasi</h3>
              <p className="text-slate-600 leading-relaxed">Laporan masuk ke Dasbor Admin Desa, diverifikasi urgensinya, dan diteruskan ke perangkat desa untuk ditindaklanjuti.</p>
            </div>

            {/* Step 3 */}
            <div className="p-8 rounded-2xl bg-slate-50 border border-slate-100 hover:shadow-lg transition duration-300">
              <div className="w-14 h-14 bg-green-100 text-green-600 rounded-xl flex items-center justify-center text-xl font-black mb-6">3</div>
              <h3 className="text-xl font-bold text-slate-900 mb-3">Tindak Lanjut Selesai</h3>
              <p className="text-slate-600 leading-relaxed">Laporan selesai dikerjakan, status berubah hijau, dan desa akan terbebas dari masalah yang diberitahukan.</p>
            </div>
          </div>
        </div>
      </section>
